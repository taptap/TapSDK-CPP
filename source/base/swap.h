...
template <typename S, typename T, typename F> S operator&(const swap_struct_t<T, F> v, const S& i) {
...
template <typename T> struct swap_64_t {
    static T swap(T x) { return static_cast<T>(tapsdk::swap64(x)); }
};

template <typename T> struct swap_32_t {
    static T swap(T x) { return static_cast<T>(tapsdk::swap32(x)); }
};

template <typename T> struct swap_16_t {
    static T swap(T x) { return static_cast<T>(tapsdk::swap16(x)); }
};

template <typename T> struct swap_float_t {
    static T swap(T x) { return static_cast<T>(tapsdk::swapf(x)); }
};

template <typename T> struct swap_double_t {
    static T swap(T x) { return static_cast<T>(tapsdk::swapd(x)); }
};
...
using u16_le = AddEndian<tapsdk::u16, LETag>::type;
using u32_le = AddEndian<tapsdk::u32, LETag>::type;
using u64_le = AddEndian<tapsdk::u64, LETag>::type;

...
template <class T, class A> struct is_trivially_copy_constructible<std::vector<T, A>>
        : std::false_type {};
#endif
}  // namespace detail
}  // namespace tl
...
template <class T, class U> struct is_adl_swap_noexcept
...
template <class T, class U> using is_void_or =
        conditional_t<std::is_void<T>::value, std::true_type, U>;

template <class T> using is_copy_constructible_or_void =
        is_void_or<T, std::is_copy_constructible<T>>;

template <class T> using is_move_constructible_or_void =
...
template <class T, class E> struct expected_copy_base<T, E, false>
...
template <class T, class E> struct expected_copy_assign_base<T, E, false>
...
template <class T, class E> struct expected_move_assign_base<T, E, false>
...
public:
...
#else
    template <class F> TL_EXPECTED_11_CONSTEXPR auto and_then(
...
#ifndef TL_EXPECTED_NO_CONSTRR
    template <class F> constexpr auto and_then(F&& f) const&& -> decltype(and_then_impl(
    ...
    template <class F> TL_EXPECTED_11_CONSTEXPR decltype(expected_map_impl(std::declval<expected>(),
                                                                           std::declval<F&&>()))
    map(F&& f) && {
        return expected_map_impl(std::move(*this), std::forward<F>(f));
    }
...
#else
    template <class F> TL_EXPECTED_11_CONSTEXPR decltype(map_error_impl(std::declval<expected&>(),
                                                                        std::declval<F&&>()))
    map_error(F&& f) & {
        return map_error_impl(*this, std::forward<F>(f));
    }
    ...
    template <class U,
              class... Args,
              detail::enable_if_t<std::is_nothrow_constructible<T,

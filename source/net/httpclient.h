...
namespace tapsdk::net {
    ...
    template<class T> using OnSuccess = const std::function<void(
    ...
    public:
        ...
        void PostAsync(const WebPath &path,
                       Headers headers,
                       Params params,
                       OnSuccess<T> success,
                       OnFailed failed) {
            RequestAsync(
                    ...
                    [success](const std::string &content) {
                        success(std::make_shared<T>(content));
                    },
                    failed);
        }

        template<JsonResult T>
        void GetAsync(const WebPath &path,
                      Headers headers,
                      Params params,
                      OnSuccess<T> success,
                      OnFailed failed) {
        ...
        template<JsonResult T>
        ResultAsync<std::shared_ptr<T>> PostAsync(const WebPath &path,
                                                  Headers headers,
                                                  Params params) {
            auto res = co_await RequestAsync(
                    POST,
                    path,
                    headers,
                    params);
        ...
        template<JsonResult T>
        ResultAsync<std::shared_ptr<T>> GetAsync(const WebPath &path,
                                                  Headers headers,
                                                  Params params) {
            auto res = co_await RequestAsync(
                    GET,
